...
class ModelInference:
    ...
    def __init__(
        ...
        if model is None:
            ...
            self.model = ClassicModel.load_from_checkpoint(
                model_filename,
        ...
        while len(seqs_ongoing["offsets"]) > 1:
            ...
            seqs_finished.extend(
                [
                    new_sequences[i]
                    for i in range(num_seqs)
                    if churn_state[i] == 1 or last_date[i] >= datetime2float(end_date)
                ]
            )
    ...
    def extend_sequence(
        ...
        for i, batch in enumerate(loader):
            encoded_batch = to_device(batch, self.device)
            # encoded_batch["t"] = tensor2npdatetime(batch["t"])

            token_event = torch.zeros(len(batch["t"]), device=self.device)
            token_event[batch["offsets"][:-1]] = 1
            encoded_batch["token_event"] = token_event

            encoded_batch["churn"] = torch.zeros(len(batch["t"]), device=self.device)
