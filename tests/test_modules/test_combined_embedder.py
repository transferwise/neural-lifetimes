...
class TestEmbedderConstruction:
    @staticmethod
    def _get_embedder(*args, **kwargs) -> CombinedEmbedder:
        return CombinedEmbedder(*args, **kwargs)
    ...
    def test_constructor(self, continuous_features, category_dict, embed_dim, drop_rate, pre_encoded):
    ...
    def test_parameter_dict(self, embed_dim, drop_rate, pre_encoded=True):
        emb = self._get_embedder([], {}, embed_dim, drop_rate, pre_encoded)
        pars = emb.build_parameter_dict()
        expected = {
            "embed_dim": embed_dim,
            "embedder_drop_rate": drop_rate,
            "pre_encoded_features": pre_encoded,
        }
